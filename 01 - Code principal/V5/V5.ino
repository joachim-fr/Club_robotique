...
#define vitesseMaximale            245     // Rapport cyclique PWM maximal pour le moteur
#define vitesseRotation            245     // Rapport cyclique PWM maximal pour la rotation
...
// variables pour la nouvelle détection des couleurs : 
int BLANC = 200;
int NOIR = 2500;
...
void setup() {
  ...
  qtrd.setTypeRC();
  qtrd.setSensorPins((const uint8_t[]){capteurD4, capteurD3, capteurD2, capteurD1}, SensorCount);
...
void loop() {
  ...
  int pos_gauche = qtrg.readLineBlack(sensorValuesG);
  Serial.print(pos_gauche);

  Serial.print('\t');

  int pos_droite = qtrd.readLineBlack(sensorValuesD);
  Serial.print(pos_droite);
    
  Serial.println();

  if (pos_gauche == 1500 && pos_droite == 1500) {
    Serial.println("STOP");
    arreter();
    mesured();
    mesureg();
    ...
    if(isGreenD && isWhiteG){
  ...
  } else if (pos_droite >= 2700 && pos_gauche == 3000) {
     ...
     while (digitalRead(capteurG2) == 0 && digitalRead(capteurG1) == 0 && digitalRead(capteurG3) == 0 && digitalRead(capteurG4) == 0) {
        tournerdroiteextremite(93);
      }
   } else if (pos_gauche <= 200 && pos_droite == 0) {
     ...
     while (digitalRead(capteurD2) == 0 && digitalRead(capteurD1) == 0 && digitalRead(capteurD3) == 0 && digitalRead(capteurD4) == 0) {
        tournergaucheextremite(93);
      }
   } else if (pos_gauche > 200 && pos_gauche <= 800 && pos_droite == 0) {
     Serial.println("Tourner à gauche 90%");
     tournergauche(95);
   } else if (pos_gauche == 3000 && pos_droite < 2700 && pos_droite >= 2200) {
     Serial.println("Tourner à droite 90%");
     tournerdroite(95);
   } else if (pos_gauche <= 1100 && pos_gauche > 800 && pos_droite == 0) {
     Serial.println("Tourner à gauche 80%");
     tournergauche(92);
   } else if (pos_gauche == 3000 && pos_droite < 2200 && pos_droite >= 1700) {
     Serial.println("Tourner à droite 80%");
     tournerdroite(92);
   } else if (pos_gauche <= 1500 && pos_gauche > 1100 && pos_droite == 0) {
     Serial.println("Tourner à gauche 70%");
     tournergauche(92);
   } else if (pos_gauche == 3000 && pos_droite < 1700 && pos_droite >= 1500) {
     Serial.println("Tourner à droite 70%");
     tournerdroite(92);
   } else if (pos_gauche == 3000 && pos_droite == 0) {
...
void tournergaucheextremite(float facteur) {  
  ...
  changeVitesseMoteurPontA(vitesseRotation * facteur);
  configurerSensDeRotationPontB(MARCHE_AVANT);
  changeVitesseMoteurPontB(vitesseRotation * facteur * 0.87);
}
...
void tournerdroiteextremite(float facteur) {  
  ...
  configurerSensDeRotationPontA(MARCHE_AVANT);
  changeVitesseMoteurPontA(vitesseRotation * facteur * 0.87);
  configurerSensDeRotationPontB(MARCHE_ARRIERE);
  changeVitesseMoteurPontB(vitesseRotation * facteur);
}
