...
#define vitesseMinimale            20       // Rapport cylique minimal du signal PWM, pour faire tourner le moteur au minimum de sa vitesse (en pratique, on évitera de trop approcher la valeur  0)
...
int value_A1 = analogRead(IN_A1);
  ...
  if (blancgauche == 1 && blancdroit == 1){
    avant();
  }

  if (blancgauche == 0 && blancdroit == 1){
      tournergauche();
    }
   

  if (blancgauche == 1 && blancdroit == 0){
    tournerdroite();
    }
  if (blancgauche == 0 && blancdroit == 0){
    avant();
    }
}
