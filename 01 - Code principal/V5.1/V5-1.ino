...
void setup() {
  ...
  qtr.setTypeRC();
  qtr.setSensorPins((const uint8_t[]){capteur1, capteur2, capteur3, capteur4, capteur5, capteur6}, SensorCount);
  qtr.setEmitterPin(4);
...
void loop() {
  ...
  if (position == 2500 && sensorValues[0] >= 150 && sensorValues[5] >= 150) {
    Serial.println("STOP");
    arreter();
    mesured();
    mesureg();
